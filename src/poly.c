...
void polyeta2_pack(uint8_t *r, const poly *a)
{
  ...
  for (i = 0; i < N / 8; ++i)
  {
    t[0] = 8 - a->coeffs[8 * i + 0];
    t[1] = 8 - a->coeffs[8 * i + 1];
    t[2] = 8 - a->coeffs[8 * i + 2];
    t[3] = 8 - a->coeffs[8 * i + 3];
    t[4] = 8 - a->coeffs[8 * i + 4];
    t[5] = 8 - a->coeffs[8 * i + 5];
    t[6] = 8 - a->coeffs[8 * i + 6];
    t[7] = 8 - a->coeffs[8 * i + 7];
...
void polyeta2_unpack(poly *r, const uint8_t *a)
{
  ...
  for (i = 0; i < N / 8; ++i)
  {
    ...
    r->coeffs[8 * i + 0] = 8 - r->coeffs[8 * i + 0];
    r->coeffs[8 * i + 1] = 8 - r->coeffs[8 * i + 1];
    r->coeffs[8 * i + 2] = 8 - r->coeffs[8 * i + 2];
    r->coeffs[8 * i + 3] = 8 - r->coeffs[8 * i + 3];
    r->coeffs[8 * i + 4] = 8 - r->coeffs[8 * i + 4];
    r->coeffs[8 * i + 5] = 8 - r->coeffs[8 * i + 5];
    r->coeffs[8 * i + 6] = 8 - r->coeffs[8 * i + 6];
    r->coeffs[8 * i + 7] = 8 - r->coeffs[8 * i + 7];
  }
#endif
...
void polyw1_pack(uint8_t *r, const poly *a)
{
  ...
  for (i = 0; i < N / 8; ++i)
  {
    r[5 * i + 0] = a->coeffs[8 * i + 0];
    r[5 * i + 0] |= a->coeffs[8 * i + 1] << 5;
    r[5 * i + 1] = a->coeffs[8 * i + 1] >> 3;
    r[5 * i + 1] |= a->coeffs[8 * i + 2] << 2;
    r[5 * i + 1] |= a->coeffs[8 * i + 3] << 7;
    r[5 * i + 2] = a->coeffs[8 * i + 3] >> 1;
    r[5 * i + 2] |= a->coeffs[8 * i + 4] << 4;
    r[5 * i + 3] = a->coeffs[8 * i + 4] >> 4;
    r[5 * i + 3] |= a->coeffs[8 * i + 5] << 1;
    r[5 * i + 3] |= a->coeffs[8 * i + 6] << 6;
    r[5 * i + 4] = a->coeffs[8 * i + 6] >> 2;
    r[5 * i + 4] |= a->coeffs[8 * i + 7] << 3;
  }
#elif GAMMA2 == (Q - 1) / 32
  for (i = 0; i < N / 2; ++i)
  ...
  for (i = 0; i < N / 8; ++i)
  {
    r[3 * i + 0] = (a->coeffs[8 * i + 0] >> 0) | (a->coeffs[8 * i + 1] << 3) | (a->coeffs[8 * i + 2] << 6);
